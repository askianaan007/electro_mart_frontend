...
export function InvestorStatementDialog({
  open,
  onOpenChange,
  investor,
}: {
  ...
  function handleGenerate() {
    ...
    onOpenChange(false);
    router.push(`/admin/investors/${investor.id}/statement?from=${fromMonth}&to=${toMonth}`);
  }

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent title="Generate investor statement">
        <DialogHeader>
          <DialogTitle>Generate Statement</DialogTitle>
          <DialogDescription>
            {investor
              ? `Choose the period to include for ${investor.name} — capital contributions and withdrawals in that range, plus their current equity position.`
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <Label htmlFor="investor-statement-from">From</Label>
            <input
              id="investor-statement-from"
              type="month"
              value={fromMonth}
          ...
          <div className="space-y-1.5">
            <Label htmlFor="investor-statement-to">To</Label>
            <input
              id="investor-statement-to"
              type="month"
              value={toMonth}
